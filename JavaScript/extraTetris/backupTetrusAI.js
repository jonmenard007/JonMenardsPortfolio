        ...
        checkForEmptyBelow2(copyObject){
            let score = 0;
            let board = this.addToBoard(copyObject)
            let length = copyObject.length
            for(let objectSpot = 0; objectSpot < length; objectSpot++){
                if(copyObject[objectSpot].row < 19){
                    let spot = (copyObject[objectSpot].row * 10) + 10 + copyObject[objectSpot].column;
                    if(board[spot].box == undefined){
                            score += Math.ceil(spot / 10) + (spot % 10);  
                    }
                }
            }
            return score;
        ...
        score(copyObject,currentObject,row){
            let moveScore = 0;
          moveScore += this.getRowScore(copyObject,currentObject,row) * this.aa;
           moveScore -= this.checkForEmptyBelow(copyObject) * this.bb;
        //   moveScore -= this.checkForEmptyBelow2(copyObject);
        //   moveScore -= this.aiCheckForHole(copyObject,this.board * this.cc);
        //     moveScore += this.aiCheckinRow(copyObject) * this.dd;
            let aibumpines_skyScore = this.aibumpines(copyObject);
        //    moveScore -=  aibumpines_skyScore[0] * this.ee;
            moveScore -= this.aiColumns(copyObject) * this.ff;
            moveScore -= aibumpines_skyScore[1] * this.gg
            return moveScore;
        }

        aiCheckForHole(currentObject,b){
            let isHole = false;
            let board = JSON.parse(JSON.stringify(this.addToBoard(currentObject)));
            let boards = [];
            for(let j = 0; j < currentObject.length; j++){
                boards[j] = JSON.parse(JSON.stringify(board));
                let spot = currentObject[j].column + currentObject[j].row * 10;
                for(let row = -1; row < 2; row++){
                    for(let column = -1; column < 2; column++){
                        if(spot % 10 == 0 && column == -1){
                            continue;
                        }
                        if((spot+1) % 10 == 0 && column == 1){
                            continue;
                        }
                        if(row == 0 && column == 0){
                            continue;
                        }                    
                        let checkedSpot = spot + (row * 10) + column
                        if(checkedSpot >= 0 && checkedSpot <= 199){
                            if(board[checkedSpot].box == undefined ){
                                let cBoard = JSON.parse(JSON.stringify(boards[j]));
                                let currentBoardHole = this.checkHole(checkedSpot,cBoard,board);                                
                                if(currentBoardHole){
                                    isHole = currentBoardHole;
                                    boards[j] = cBoard;
                                }else{
                                    boards[j] = JSON.parse(JSON.stringify(board));
                                }
            ...
            if(isHole == true){     
                let holeCount = 0
                for(let spot = 199; spot >= 0; spot--){
                    for(let objectSpot = 0; objectSpot < 4; objectSpot++){
                        if(boards[objectSpot][spot].box == "empty"){
                            holeCount++;
                            break;
                        } 
                    }
        ...
        //done
        checkHole(spot,board,orignalBoard){
            let oBoard = orignalBoard;
            board[spot].box = "empty";
            let foundEmpty = true;
            for(let row = -1; row < 2; row++){
                for(let column = -1; column < 2; column++){
                    let checkedSpot = spot + (row * 10) + column;                    
                    if(spot % 10 == 0 && column == -1){
                        continue;
                    }else if((spot+1) % 10 == 0 && column == 1){
                        continue;
                    }else if(row == 0 && column == 0){
                        continue;
                    }else if(checkedSpot >= 0 && checkedSpot <= 199){
                        if(board[checkedSpot].box == undefined || board[checkedSpot].box == "cleared"){
                            if(checkedSpot < 30 || board[checkedSpot].box == "cleared"){
                                for(let spot = 199; spot >= 0; spot--){
                                    if(board[spot].box == "empty"){
                                        oBoard[spot].box = "cleared"   
                                    }  
                                }
                                return false;
                            }
                            foundEmpty = this.checkHole(checkedSpot,board,oBoard);
                            if(!foundEmpty){
                                return foundEmpty;
                            }
                        }
        ...
        aiCheckinRow(currentObject){
            ...
            let length = currentObject.length
            for(let objectSpot = 0; objectSpot < length; objectSpot++){
                if(currentObject[objectSpot].row > maxRow){
                    maxRow = currentObject[objectSpot].row;
                }
                if(currentObject[objectSpot].row < minRow){
                    minRow = currentObject[objectSpot].row;
                }
            }
            for(let checkRow = minRow; checkRow < maxRow; checkRow++){
                for(let column = 0; column < 10; column++){
                    if(board[checkRow * 10 + column].box == undefined){
                        counterEmpty++;
                    }else{
                        countFull++;
                    }
            ...
            return  (rowsFilled * 15) + (-1 * counterEmpty)
        }
        ...
        aibumpines(currentObject){
            ...
            for(let column = 0; column < 10; column++){
                for(let row = 0; row < 20; row++){
                    ...
                    if(board[column + (row * 10)].box == undefined){
                        counter++;
                    }else{
                        results[column] = counter;
                        break;
                    }
                }
                if(results[column] == undefined){
                    results[column] = 20
                }
                counter = 0 
            }
            let min = Math.min.apply(Math,results);
            let max = Math.max.apply(Math,results);           
            return [max - min,countSkyBlocks];
        }        
        ...
        placeOneObject(a,b,c,d,e,f,g){
            ...
            for(let i = 0; i < combinedpossibleSpots.length -1; i++){
                ...
                }else if(combinedpossibleSpots[i].length == 0){
                    combinedpossibleSpots.splice(i,1);
                }else{
           ...
           // console.log(combinedpossibleSpots)
            ...
            for(let i = 0; i < combinedpossibleSpots.length -1; i++){
                ...
                let returnValue = this.findPath(combinedpossibleSpots[i][2],0,0,0,0,0,combinedpossibleSpots[i][3])
       ...
       //    console.log("gameOver")
        ...
        findPath(object,depthCount,left,right,rotate, last,swap){
           
            // possible moves are down, left, right, rotate, swap, place
            // take a picture of the board, goal is to fill a row, place, low, not have spaces.
            // this function is recursive and will take the highest score
            let currentObject = JSON.parse(JSON.stringify(object));;            
            // results will be what is returned, a combination of moves and score
            let results = []
            results[0] = false;
            // moves will be the best set of moves done by the different possibilites
            let moves = [];
            let total = 0;
            //1 = move left
            //2  = move right
            ...
            //4  = rotate
            //5  = 32 swap hold
            if(depthCount < 22 && right < 7 && left < 7 && rotate < 6){
                ...
                if(correctPositions != 4){
                   // this.addToBoard();
                    let spaceAbove;
                    if(depthCount == 0){
                        spaceAbove = this.aiSpacesAbove(currentObject) - 5;
                        if(spaceAbove < 0){
                            spaceAbove = 0;
                        }
                    ...
                    for(let i = 5; i >= 1; i--){
                        ...
                        if(i == 5){
                            ...
                            if(spaceAbove2 != 0){
                                ...
                                let res = this.findPath(currentObjectClone2,depthCount + 1,left + moveLeft,right + moveRight,rotate + rotating,i,false);
                                total += res[2];
                                if(res[0] == true){
                            ...
                            if(this.aiMoveTile(i,currentObjectClone)){
                                ...
                                let res = this.findPath(currentObjectClone,depthCount + 1,left + moveLeft,right + moveRight,rotate + rotating, i,false);
                                total += res[2];
                                if(res[0] == true){
                                    res[1] = myMoves.concat(res[1]);
                                    total += depthCount
