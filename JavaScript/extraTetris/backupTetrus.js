       ...
       //astetics for the game
        ...
        function ai(){           
                ...
                if(currentObject != undefined){
                    ...
                    let copyHoldObject = JSON.parse(JSON.stringify(holdObject));

                    for(let i = 0; i < copyCurrentObject.length; i++){
                        copyBoard[copyCurrentObject[i].row * 10 + copyCurrentObject[i].column].box = undefined;
                    }

                    tetrus = new aiTetrus(copyBoard,copyCurrentObject,copyHoldObject);
                    let result = tetrus.placeOneObject(a,b,c,d,e,f,g);
                    if(result === false){
